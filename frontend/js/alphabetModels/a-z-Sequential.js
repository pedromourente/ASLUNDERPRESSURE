...
function onResults(results) {
  ...
  if (results.multiHandLandmarks.length > 0) {
    ...
    console.log(alphabet[gestureIndex]);
    ...
    if (gestureIndex === alphabetIdx && alphabetIdx < alphabet.length - 1) {
      ...
      alphabetIdx += 1;
      outputImg.src = `/assets/asl_alphabets/${alphabet[alphabetIdx]}hand.png`;
    }
    if (gestureIndex === alphabetIdx && alphabetIdx === alphabet.length - 1) {
      showNum.classList.add("hidden");
      correctAns.classList.remove("hidden");
      ...
      alphabetIdx = 0;
      outputImg.src = `/assets/asl_alphabets/${alphabet[0]}hand.png`;
    }
  }
  canvasCtx.restore();
}
...
async function loadModel() {
  customModel = await tf.loadLayersModel(
    "https://raw.githubusercontent.com/pedromourente/ASLUNDERPRESSURE/main/frontend/model/alphabet/model.json"
  );
}
